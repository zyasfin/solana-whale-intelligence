...
use solana_whale_intelligence::sf::token::TokenLifecycle;
...
fn accept3_funded_by_known_deployer_is_reconstructed() {
    ...
    let edges = vec![
        edge(EdgeType::FundedBy, "wallet:D1", "wallet:NEW1", &["ev-fund"]),
        // Reused-social edge: the new wallet reuses the anchor's immutable X id.
        edge(EdgeType::ReusedSocialLink, "wallet:NEW1", "x:acct_immutable_1", &["ev-reuse"]),
    ];
    let out = resolve_candidates(&anchor(), &nodes, &edges, NOW);
    assert_eq!(out.len(), 1);
...
fn accept3b_funding_alone_is_not_reconstructed() {
    ...
    let out = resolve_candidates(&a, &nodes, &edges, NOW);
...
fn accept4_reused_link_without_ownership_is_insufficient() {
    ...
    let out = resolve_candidates(&anchor(), &nodes, &edges, NOW);
    // Reuse resolves to the OTHER token (sol:BBB), classified Insufficient.
...
fn accept10_dormant_tokens_no_polling() {
    use solana_whale_intelligence::sf::recent::ActivationTrigger::*;
