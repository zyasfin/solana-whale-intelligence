...
pub fn is_edge_valid(edge: &EntityEdge, now_secs: i64) -> bool {
    let from = parse_secs(&edge.valid_from).unwrap_or(i64::MAX); // unparseable -> invalid
    if now_secs < from {
        return false;
    }
...
pub fn is_false_confluence(a: &EntityEdge, b: &EntityEdge) -> bool {
    ...
    // Low confidence and non-Confirmed truth status -> suspicion.
    let low_confidence = a.confidence.unwrap_or(0.0) < 0.5 && b.confidence.unwrap_or(0.0) < 0.5;
