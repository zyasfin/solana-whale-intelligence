...
pub async fn report(
    ...
    let report = json!({
        ...
        "chains": chains_json,
        "helius": {
            "providers": helius_provider_count,
            // Provider IDs are positional (helius_1..N); never the keys.
            "provider_ids": (1..=helius_provider_count).map(|i| format!("helius_{i}")).collect::<Vec<_>>(),
        },
        "gmgn": {
            "configured": gmgn_configured,
            "bucket_tokens": gmgn_bucket_tokens,
        },
        "telegram": {
