    ...
    match cli.command {
        ...
        Command::Token { action } => match action {
            TokenAction::Discover { once: _ } => {
                ...
                let gmgn = gmgn::GmgnPool::load(pool.clone(), settings.config.gmgn.clone()).await?;
                if gmgn.is_empty() {
                    bail!("no GMGN keys configured; add keys in the admin API panel");
                }
        ...
        Command::Wallet { action } => match action {
            WalletAction::Sync { address, chain, max_pages } => {
                ...
                let helius = helius::DbHeliusPool::load(pool.clone(), settings.config.helius.clone()).await?;
                if helius.is_empty() {
                    bail!("no Helius keys configured; add keys in the admin API panel");
                }
        ...
        Command::Watch { action } => match action {
            ...
            WatchAction::ServeAdmin { bind } => {
                ...
                let admin_state = admin::AdminState::new(pool, std::sync::Arc::new(settings.clone())).await;
        ...
        Command::Run => {
            ...
            // DB-backed API key pools (admin API panel). Empty pools disable
            // the corresponding ingest paths with a clear warning.
            let helius = std::sync::Arc::new(
                helius::DbHeliusPool::load(pool.clone(), settings.config.helius.clone()).await?,
            );
            helius.spawn_usage_flusher();
            if helius.is_empty() {
                tracing::warn!("no enabled Helius keys in helius_keys; Solana ingest disabled — add keys in the admin API panel");
            }
            let gmgn = match gmgn::GmgnPool::load(pool.clone(), settings.config.gmgn.clone()).await {
                Ok(pool) if !pool.is_empty() => {
                    let pool = std::sync::Arc::new(pool);
                    pool.spawn_usage_flusher();
                    Some(pool)
                }
                Ok(_) => {
                    tracing::warn!("no enabled GMGN keys in gmgn_keys; discovery worker disabled — add keys in the admin API panel");
                    None
                }
                Err(err) => {
                    tracing::warn!(error = %err, "gmgn key pool load failed; discovery worker disabled");
                    None
                }
            };
            ...
            // EVM endpoint (uses the first quota-available key from the pool).
            let robinhood_key = helius.first_available_key();
            let robinhood_enabled = robinhood_key.is_some()
            ...
            if robinhood_enabled {
                let robinhood_rpc_url = settings.config.helius.evm_http_url(
                    ...
                    robinhood_key.as_deref().unwrap_or_default(),
                );
            ...
            // Solana filtered funding stream (first quota-available pool key).
            if let Some(helius_key) = helius.first_available_key() {
                ...
                tracing::warn!("no Helius key available; solana funding stream disabled");
            }
        ...
        Command::Health => {
            let database_url = require_database_url(&settings)?;
            let pool = db::connect(&database_url, 2).await?;
            let (helius_total, helius_enabled): (i64, i64) = sqlx::query_as(
                "SELECT COUNT(*), COUNT(*) FILTER (WHERE enabled) FROM helius_keys",
            )
            .fetch_one(&pool)
            .await
            .unwrap_or((0, 0));
            let (gmgn_total, gmgn_enabled): (i64, i64) = sqlx::query_as(
                "SELECT COUNT(*), COUNT(*) FILTER (WHERE enabled) FROM gmgn_keys",
            )
            .fetch_one(&pool)
            .await
            .unwrap_or((0, 0));
            let robinhood_configured = helius_enabled > 0
                && !settings.config.chains.robinhood.chain_id.is_empty();
            let chain_health = health::chain_health(
                ...
                if robinhood_configured {
                    ...
                    Some("no enabled Helius keys in the API panel or robinhood chain_id unset")
                },
                None,
            );
            let report = health::report(
                &pool,
                chain_health,
                helius_enabled as usize,
                helius_total as usize,
                gmgn_enabled as usize,
                gmgn_total as usize,
            )
            .await?;
